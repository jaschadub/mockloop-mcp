...
async def main():
    ...
    try:
        ...
        print(json.dumps(test_plan_result, indent=2, default=str)) # Use default=str for datetime
        print("-" * 50)

        if test_plan_result.get("status") == "success" or test_plan_result.get("status") == "partial_success":
            print("\nTest plan executed. Review the 'execution_results'.")
            print("You should see requests made to JSONPlaceholder for '/todos/{todoId}'.")
        else:
